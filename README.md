


# Architecting Systems of Systems: A Tertiary Study


## Appendix C - Identification of Research and Secondary Studies Selection

[Venues search log (Search Strings)](search-strings-log.md)

[Automated search output (bibtex) - DS1](DS1.bib)

[Automated search output (bibtex) - DS2](DS2.bib)

[Automated search output (bibtex) - DS3](DS3.bib)

[Automated search output (bibtex) - DS4](DS4.bib)

[Automated search output (bibtex) - DS5](DS5.bib)

[Final dataset (bibtex) - DS6](DS6.bib)

[Duplicated filtering and filtering consensus](FilteringConsensusAnalysis.xlsx)

## Appendix D - Quality Assessment

[Duplicated quality assessment and consensus](QA-Comparison-Disagreements.xlsx)

## Appendix E - Qualitative Data Analysis & Data Extraction

[Quotations and codes](quotations.md)

[Axial coding - quotations and relations](SoS-Arch-TS-AxialCoding.xlsx)

[Axial coding - raw quotations and codes](overlaping-concepts-quotations.md)

[Atlas.ti project bundle](SoS-Arch-TertiaryStudy-AtlasTI-Bundle.atlproj)


